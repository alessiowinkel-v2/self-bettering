...
**Status:** ✅ Done. Next-workout card is right-sized, "Start ›" with chevron as text link (correct, not filled) — same treatment on both Today and Gym Home after the Phase 3d follow-up consolidation. Routines render as rows not cards. The amber dot on the next-up routine inside the routines list is exactly the subtle hierarchy specified. "+ Add routine" reads as a quiet text link. THIS WEEK uses the small-caps label treatment that Habit Detail established, not a Fraunces section header.

Built in code in Phase 3d at [app/(tabs)/gym.tsx](../app/(tabs)/gym.tsx). Hydrates via [state/gymHomeStore.ts](../state/gymHomeStore.ts) on focus. Two new SQLite queries in [db/workouts.ts](../db/workouts.ts): `getWorkoutTemplatesWithLastCompleted` (correlated MAX per template, LEFT-JOIN-equivalent semantics, preserves rotation_order) and `getCompletedWorkoutDatesInRange` (DISTINCT date strings via `substr(completed_at, 1, 10)`). No migration. The follow-up dropped the redundant `getNextWorkoutTemplate` call from the gym hydrate; next-up derives from `templates[0]` since both queries share `ORDER BY rotation_order ASC, name ASC`. Date math + the lowercase "Last · {phrase} · {N} exercises." subtitle live in [utils/gymHome.ts](../utils/gymHome.ts) (`getWeekRange`, `buildGymWeekDots`, `formatRelativeDate`). The week strip reuses the new shared primitive [components/primitives/WeekDots.tsx](../components/primitives/WeekDots.tsx) — promoted out of `components/habit/` and the status union renamed `'held' | 'slipped' | 'empty'` → `'filled' | 'outlined' | 'empty'` so the vocabulary is domain-agnostic. `buildWeekDots` in [utils/habitDetail.ts](../utils/habitDetail.ts) maps `'held' → 'filled'`, `'slipped' → 'outlined'`, no-log → `'empty'`. Habit Detail renders identically — pure naming change. Gym Home's `buildGymWeekDots` only emits `'filled'` and `'empty'` (workouts have no slipped concept). `NextWorkoutCard` was consolidated under [components/workout/NextWorkoutCard.tsx](../components/workout/NextWorkoutCard.tsx); both Today and Gym Home import from there with the same `"Start ›"` label. The "X of N done this week." caption uses `routinesCompletedThisWeek` (distinct routines whose most recent completion falls in the Mon–Sun window) as numerator and `templates.length` as denominator — a routine done Mon and Fri still counts as one, tracking rotation coverage rather than session frequency. Routine row tap, "+ Add routine", "Start", and "Add one." all `console.log` for now; Active Workout wiring is Phase 3e+.

### Journal List
**States covered:**
- Dark · default (entries grouped by month, newest-first, each row as a JournalPreviewCard)
- Dark · empty ("No entries yet." with "Write something." TextButton routing to today's editor)
- Cross-year groups (label appends year: "MAY 2025") for entries before the most-recent entry's year

**Status:** ✅ Done. Built in Phase 3e at [app/(tabs)/journal.tsx](../app/(tabs)/journal.tsx). Hydrates screen-local via `useFocusEffect` (no store — the editor's save-time `refreshJournalSlice` already covers Today, and Journal List has no cross-screen writes). New SQLite query in [db/journal.ts](../db/journal.ts): `getAllJournalEntries` (ORDER BY date DESC; sort by content's day, not created_at, so editing a past entry doesn't jump it to the top). No migration.

Component reuse:
- The `YesterdayCard` previously under `components/today/` was promoted to [components/journal/JournalPreviewCard.tsx](../components/journal/JournalPreviewCard.tsx) — same card now serves Today's Yesterday slot and every Journal List row. Today's `(tabs)/index.tsx` imports it from `components/journal` after the move.
- `MoodDots` moved from `components/today/` → `components/journal/` alongside `InteractiveMoodDots`. Today's `JournalPreviewCard` is the only consumer of the read-only variant and now sits in the same domain folder as the editor's interactive variant.

Month dividers use a new screen-local primitive [components/journal/MonthHeader.tsx](../components/journal/MonthHeader.tsx) — Inter `label` small-caps, `tone="secondary"`, intentionally not Fraunces (a Fraunces SectionHeader reads too loud as an in-list segment; the muted small-caps treatment matches the "LAST 90 DAYS" rhythm from Habit Detail). Grouping logic lives in [utils/journalList.ts](../utils/journalList.ts) (`groupEntriesByMonth`) — the reference year is the year of the most recent entry; same-year groups render bare month names, prior-year groups append the year to avoid silently conflating two Mays.

No "+ Add entry today" link on the list — today's entry is always reachable from Today's NoJournalYetCard. Surfacing the same affordance twice would add noise.

### Settings
**States covered:**
- Dark · default (Appearance / Notifications / Data / About sections; theme selector showing system/dark/light; check-in rows currently off)
- Dark · check-in expanded (row tapped, inline DateTimePicker shows below, "Turn off." control visible when value is set)
- Dark · clear-data confirm (Alert.alert with destructive "Clear" action)
- Light · default

**Status:** ✅ Done. Built in Phase 3e at [app/settings.tsx](../app/settings.tsx). Stack route under root, reachable from Today via the cog icon top-right ([components/today/SettingsCog.tsx](../components/today/SettingsCog.tsx) — Lucide `Settings` glyph imported individually per the project's tree-shaking convention, absolute-positioned inside Today's contentContainer so the screen padding stays untouched).

Architecture:
- Theme override reads from existing [theme/useThemeStore.ts](../theme/useThemeStore.ts) — no duplication.
- New [state/settingsStore.ts](../state/settingsStore.ts) with Zustand + AsyncStorage persist carries the rest: `morningCheckInTime`, `eveningCheckInTime` ("HH:mm" strings or null when off), and `restTimerAlerts` boolean. Phase 3e ships the configuration surface only — Phase 4 wires actual notification scheduling.
- Theme tokens gained a `destructive` role ([theme/tokens.ts](../theme/tokens.ts)) — `#B5482E` dark / `#9E3B1F` light. `Text.tsx`'s `Tone` union extends to `'destructive'`; `Button.tsx`'s `tone="destructive"` placeholder finally resolves to the real token instead of falling back to primary.
- `ListRow`'s `below` slot was already present from Phase 3a — Phase 3e fleshed out its docstring with an inline-expansion example. The slot renders between the row body and the trailing divider so the divider stays beneath the entire expanded row group.
- Theme selector is an always-visible three-button row ([components/settings/ThemeChoice.tsx](../components/settings/ThemeChoice.tsx)) — the choice space is small enough that hiding alternatives behind a tap would read as friction. Selected option is amber-tone, others are `tone="secondary"`.
- Check-in rows use a screen-local pattern ([components/settings/CheckInRow.tsx](../components/settings/CheckInRow.tsx)). Right slot shows the current "HH:mm" or a muted "Off" caption — never an "Off" TextButton at the row level. Tap to expand `@react-native-community/datetimepicker` inline (iOS spinner, 24-hour). When a value is set, the expanded picker also renders a right-aligned "Turn off." TextButton (`tone="secondary"`) — the negation control lives inside the expansion, not on the collapsed row.
- Rest timer alerts is a plain `Switch` on the row's right slot; the ListRow renders without `onPress` so the Switch handles its own toggle.
- Export ([db/export.ts](../db/export.ts) — `buildExportPayload`) dumps every domain table via `SELECT *` with a top-level `schemaVersion = MAX(_migrations.version)`. Field names are camelCase, JSON-encoded columns (tags, exercises) stay as strings. Settings writes the payload to `Paths.document` as `lumen-export-{today}.json` and hands off to `expo-sharing`. Phase 4's Import will validate `schemaVersion` against the bundle's latest migration — out of scope here is how mismatches are handled, only that Export writes the field.
- Import is OMITTED entirely in Phase 3e — no disabled stub row. Phase 4 lands it.
- Clear all data uses `Alert.alert` with `style: 'destructive'`. After wipe ([db/wipe.ts](../db/wipe.ts) — single transaction, FK-respecting order, leaves `_migrations` intact), every cross-screen store (`useTodayStore`, `useHabitsListStore`, `useGymHomeStore`) is re-hydrated so empty surfaces paint correctly when the user navigates back.
- About section is just the Version row reading from `Constants.expoConfig?.version`. No credit line, no build number — the audience is the project owner; surfaces that read like an App Store about-page would feel off.
...
- **`NextWorkoutCard` "Start ›" chevron glyph weight.** Currently: literal `›` (U+203A, single right-pointing angle quotation) inline in the `TextButton` label, rendering at Inter Medium 16. Used on both Today and Gym Home — the card lives at [components/workout/NextWorkoutCard.tsx](../components/workout/NextWorkoutCard.tsx) post Phase 3d follow-up consolidation. Inter ships the chevron at a slightly-thicker weight than the system pseudo-chevron in iOS lists. Phone-test whether the inline glyph reads as "tap me" or as decorative punctuation; remediation is replacing with a Lucide `ChevronRight` icon at the same baseline.
...
- **Gym Home "X of N done this week" semantics.** Resolved in Phase 3d follow-up: numerator is **distinct routines** whose most recent completion falls within the current Mon–Sun week; denominator is total routine count. A routine done Mon and Fri still counts as one — the metric tracks rotation coverage, not session frequency. Hidden when numerator is zero. Phone-test the read on a real week (e.g. Push A done Mon, Pull A done Wed, Push A done Fri → reads "2 of 4 done this week.", not "3 of 4").
...
- **Gym Home THIS WEEK marginTop on no-NextWorkoutCard.** Currently: the THIS WEEK block wraps with `marginTop: spacing[7]`, designed for the gap below the NextWorkoutCard. In the edge case where `templates.length > 0` but `nextTemplate === null` (rotation key missing or never-completed templates only), the NextWorkoutCard block elides and `spacing[7]` becomes the gap below the screen title instead. Phone-test whether the air reads correctly in that variant, or whether the marginTop should collapse when no preceding section rendered. Likely the current behavior is fine — the edge case is rare in practice — but worth an eyeball.

- **Destructive hex on glass.** Currently: `theme.colors.destructive` resolves to `#B5482E` (dark) and `#9E3B1F` (light). Used on Settings' "Clear all data" row and on CheckInRow's "Turn off." button label. The values were chosen to sit next to amber without competing chromatically. Phone-test whether the dark hex reads as appropriately weighty against `bg: #100F0D` and whether the light hex holds contrast against `bg: #F4EFE6` (the cream is warm, so the destructive may need to deepen). If either reads too muted or too loud, dial in 5-10 points either side and re-eyeball.

- **Settings cog placement on Today.** Currently: absolute-positioned at `top: spacing[3]`, `right: 0` inside Today's ScrollView contentContainer (the contentContainer already paints its own horizontal gutter via `Screen`'s `paddingHorizontal`, so `right: 0` lands the cog flush with the right gutter rather than past it). The cog's icon is Lucide `Settings` at 22px, strokeWidth 1.5, color `textSecondary`. Phone-test whether the cog reads as a quiet affordance at the top-right or as a competing visual to the "Today." display title beside it. Remediation if too loud: drop the icon size to 20, or push opacity down via `textTertiary`.

- **Journal List month-header typography weight.** Currently: Inter `label` (11/14, small-caps spacing 0.9), `tone="secondary"`. Header lockup: bare month name for current-year groups ("MAY"), `MONTH YEAR` for prior-year groups ("MAY 2025"). Alternative considered and rejected: Fraunces SectionHeader (the canonical "Streaks" / "Yesterday" treatment) — would read too loud as a mid-list segment when the list is dense. Phone-test against a few months of entries; if the small-caps reads too thin, bump to `tone="primary"` rather than upsizing to Fraunces.

- **Journal List empty-state copy.** Currently: italic Fraunces "No entries yet." with a `TextButton` "Write something." beneath, routing to today's editor. The button is `tone="accent"`. Voice-check: "Write something." is one of the few imperative fragments in the app — phone-test whether it reads natural or whether "Today's entry." (declarative, pointing at the destination rather than commanding the action) would sit better with the voice rules.

- **Settings check-in row inline picker height on iOS.** Currently: `DateTimePicker` in spinner mode renders inline inside the row's `below` slot. The iOS spinner is ~200px tall — when expanded, the row stretches significantly. Phone-test whether the expanded row reads as comfortably tall or whether the spinner needs `display="compact"` to sit beside the row instead of below it. The picker's `accentColor` is wired to `theme.colors.accent` so amber drives the highlight, but the spinner's neutral text contrast is iOS-determined — eyeball whether the wheels read against the screen bg in both themes.

- **Settings check-in "Turn off." inside expanded picker.** Currently: TextButton `tone="secondary"` aligned to the picker's right edge, below the spinner. Phone-test whether `tone="destructive"` reads more clearly as the negation — the destructive hex is loud, but "Turn off." against the picker's neutral chrome could read as just another time-related label without the chromatic cue. The current choice keeps the row visually quiet, but on-glass we may want a touch more weight.

- **Settings Clear-all-data row tone.** Currently: row label is `tone="destructive"` (resolves to the new amber-rust hex). The row is otherwise standard `ListRow` with `onPress`, no inset, no separator emphasis. Phone-test whether the destructive tone on the label alone reads as "this is dangerous" or whether a separator break (e.g. spacing[5] gap above the row) would help separate it from the neighboring "Export to JSON" row. Alternative: a small "DANGER" small-caps label above the row — too heavy for the app's voice, but flag-able if the row reads as undifferentiated.

- **Settings sectionheader spacing on the first section.** Currently: the "Appearance" SectionHeader passes `marginTop={0}` (the title's own marginBottom carries the gap). All subsequent SectionHeaders default to `marginTop: spacing[6]`. Phone-test whether the visual rhythm reads consistently — the design system's canonical SectionHeader margin is `spacing[6]` top, but the first one needs to collapse to avoid stacking with the title's marginBottom. If the first section sits too tight against the title, lift to `marginTop: spacing[3]` or `spacing[2]` rather than fully eliding.

- **SettingsCog vs TodayHeader collision on long dates.** Currently: the cog sits absolute at `top: spacing[3]`, `right: 0` and the date-greeting line in `TodayHeader` flows full-width below the "Today." display. On a long weekday like "Wednesday, December 24 Evening." the right edge of the date line could approach or overlap the cog's left bound. Phone-test on the longest weekday-month combinations (Wednesday + September/November/December). Remediation if collision occurs: shrink the cog (size 18), or constrain `TodayHeader`'s date line to `paddingRight: spacing[8]` so it terminates well clear of the cog.

- **Theme picker three-button row reads as tappable or as static labels.** Currently: three `TextButton`s side-by-side, selected = `tone="accent"`, unselected = `tone="secondary"`. No fill, no border, no underline. Phone-test whether the row reads as three distinct affordances or as a single label-and-state lockup that doesn't invite tapping. Remediation if it reads static: a hairline underline beneath the selected option, or wrapping each in a `Pressable` with a subtle pressed background.

- **Settings inline DateTimePicker blocks scroll access to Data/About.** Currently: when a CheckInRow is expanded the iOS spinner takes ~200px of vertical room. With both Morning and Evening expanded the picker stack pushes Data and About below the fold, and if the user can't scroll past the picker the lower sections become unreachable. Phone-test whether the Settings ScrollView still pans freely with a picker open, or whether the spinner intercepts vertical drag gestures. Remediation if blocked: switch to `display="compact"` (renders a small chip that opens iOS's native modal sheet on tap), which yields the vertical space back.
